...
class ApiDio {
  ...
  static void _handelServerError(DioException e) {
    if (_serverError != null) {
      Future.microtask(() {
        try {
          debugPrint('Error in server error listener: $e');
          _serverError!();
        } catch (error) {
          debugPrint('Error in server error listener: $error');
        }
      });
    } else {
      debugPrint('No server error listener registered');
    }
  }
  ...
  Future<ApiResponse> request({
    ...
    try {
      ...
      // debugPrint(
      //     '###@@@ request response: ${response.statusCode} ${response.statusMessage}');
      // debugPrint('###@@@ request response data: ${response.data.toString()}');

      if (response.statusCode == 200) {
        _handleLoginStatus(response.data);

        if (response.data["statusCode"] == 302) {
          debugPrint('###@@@ request response 302');
          if (loginErrorHandling != true) {
            _handleErrorWithData(response.data!);
            debugPrint('302: ${response.statusCode} ${response.statusMessage}');
          }
          return ApiResponse(
        ...
        if (response.data["statusCode"] == 403 ||
            ...
            response.data["statusCode"] == 409 || // 프로젝트 제한 초과
            response.data["statusCode"] == 413 || // 저장 용량 제한 초과
